...
ISBN自動検索機能のテストをAIに生成させてください。実際の外部サービスを呼び出して結果を検証するテストを、既存のテストと同じパターンで作成するよう伝えましょう。
...
### ステップ3: テストの安定性を確認する（15分）

テストがパスしたとしても、外部サービスに依存するテストがCI環境で安定して動作するとは限りません。テストを複数回実行して、安定性を確認しましょう。
...
> **問いかけ:**
> - 5回の実行結果はすべて同じでしたか？
> - 結果が不安定な場合、原因はどこにあると思いますか？
> - このように実行するたびに結果が変わるテストを **Flaky Test（不安定テスト）** と呼びます
> - Flaky TestがCI環境にあると、チームにどのような影響がありますか？
...
> - FIRST原則の **R（Repeatable: 繰り返し可能）** の観点で、このテストの問題を説明してください
> - 不安定なのは「テスト対象のコード（BookService）」ですか、それとも「テストの書き方」ですか？
