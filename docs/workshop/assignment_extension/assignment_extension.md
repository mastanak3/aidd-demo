...
> **ヒント:** こんな機能を持たせると便利です
> - コンストラクタやセッターで「返すべき `Loan`」を設定できる
> - 呼び出されたかどうかを記録できる
...
| # | テストケース | 期待結果 |
|---|------------|---------|
| 1 | 延長データをDBに保存する | 更新された `Loan` が返る（`dueDate` +7日、`extended` が `true`） |

> **ヒント:** テストのイメージ
> ```java
> // 1. フェイクを用意して更新済みの Loan を返すように設定
> // 2. フェイクの saveExtension メソッドを呼ぶ
> // 3. 結果の dueDate と extended を検証する
> ```
...
2. `extendLoan()` の中で以下の処理を実装する:
   ...
   - 延長済みでないことを確認する
   - 返却期限を7日延長した日付を計算する
   - `loanExtensionPort.saveExtension(...)` でDBに保存する
   - 結果を返す
...
> return loanExtensionPort.saveExtension(loanId, newDueDate);
> ```
...
> - フェイクの `LoanExtensionPort` は、渡された `loanId` と `newDueDate` を使って更新済みの `Loan` を返します
